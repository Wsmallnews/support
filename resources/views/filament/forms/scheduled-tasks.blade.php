                ...
                <div class="w-full flex flex-col gap-2">
                    ...
                    <div class="sn-descript-text flex items-center gap-1">
                        @if ($task->executed_at)
                            ...
                            {{ __('sn-support::scheduled_task.timeline.executed_at') }}: {{ $task->executed_at }}
                        @else
                            {{ __('sn-support::scheduled_task.timeline.no_executed') }}
                        @endif
                    </div>

                    @if (filled($task->payload))
                        <div class="sn-tip-text">
                            {{ __('sn-support::scheduled_task.timeline.payload') }}: {{ json_encode($task->payload, JSON_UNESCAPED_UNICODE) }}
                        </div>
                    @endif
